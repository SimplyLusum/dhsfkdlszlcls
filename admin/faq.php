<?php 
    ...
    /* get groups */
    $sql = "select grp_id, grp_name from tbl_group_grp order by grp_order asc";
	...
	switch ($action) {
		case "view":
			...
			$df->set_field(array("Question","faq_title",500));
			$df->set_icon(array("status","order","edit","delete"));
			$df->set_page_size(30);
			$df->set_order(false,"faq_order","asc");
	
			$sql="select * from tbl_faq_faq where group_id = ".$faq_type;
			$url="?pg=$pg";
		...
		case "edit":		
			...
			$df->set_form("text","Question","faq_title",$text['faq_title'],"class=\"text-large\"","");
			$df->set_form("textarea","Answer","faq_desc",$text['faq_desc'],"class=\"textarea-full\" style=\"height:200px\"",""); 
